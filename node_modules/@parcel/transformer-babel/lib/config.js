...
async function load(config, options, logger) {
  ...
  if (partialConfig == null) {} else if (partialConfig.hasFilesystemConfig()) {
    ...
    if (configIsJS) {
      ...
      config.setResolvedPath(typeof babelrcPath === 'string' ? babelrcPath : configPath);
    }
...
async function reload(config, options) {
  let {
    loadPartialConfig
  } = await loadBabelCore(config, options);
...
function loadBabelCore(config, options) {
  var _config$result;

  return !config.isSource || ((_config$result = config.result) === null || _config$result === void 0 ? void 0 : _config$result.internal) ? bundledBabelCore : options.packageManager.require('@babel/core', config.searchPath);
}