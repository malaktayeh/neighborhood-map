...
var _default = new _plugin.Transformer({
  async transform({
    ...
    let gql = await options.packageManager.require('graphql-tag', asset.filePath);
    let gqlMap = new Map();

    const traverseImports = (name, assetCode) => {
      gqlMap.set(name, assetCode);
      return Promise.all(assetCode.split(NEWLINE).map(line => line.match(IMPORT_RE)).filter(match => !!match) // $FlowFixMe
    ...
    asset.type = 'js';
    asset.setCode(`module.exports=${JSON.stringify(gql([...gqlMap.values()].join('\n')), null, 2)};`);
    return [asset];
  }
